...
export default function ToolInfo({ title, description, features, steps, privacyInfo }: ToolInfoProps) {
 return (
  <div className="w-full max-w-5xl mx-auto mt-20 space-y-16 pb-16">
   ...
   <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
    {features.map((feature, index) => (
     <div key={index} className="bg-gray-800/50 border border-gray-700 rounded-xl p-6 hover:border-amber-500/50 transition-colors group">
   ...
   <div className="bg-gray-800/30 border border-gray-700/50 rounded-2xl p-8 md:p-12">
    <div className="flex items-center space-x-3 mb-8">
     <div className="w-10 h-10 bg-amber-700 rounded-full flex items-center justify-center text-amber-100 font-bold">?</div>
     <h2 className="text-2xl font-bold text-gray-100">How to use {title}</h2>
    </div>
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
     {steps.map((step, index) => (
      <div key={index} className="relative">
       {index < steps.length - 1 && <div className="hidden lg:block absolute top-5 left-1/2 w-full h-[2px] bg-gray-700 -z-10" />}
